...
object TimeUsage extends TimeUsageInterface {
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def isPrimary(col: String): Boolean = primary.exists(col.startsWith(_))
    def isWorking(col: String): Boolean = working.exists(col.startsWith(_))
    def isOther(col: String): Boolean = other.exists(col.startsWith(_))
    (
      columnNames.filter(col => isPrimary(col)).map(column),
      columnNames.filter(col => isWorking(col)).map(column),
      columnNames.filter(col => isOther(col) && !(isPrimary(col) | isWorking(col))).map(column)
    )
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
      ...
      .map(row => TimeUsageRow(
          ...
          BigDecimal(row._2).setScale(1, BigDecimal.RoundingMode.HALF_UP).toDouble,
          BigDecimal(row._3).setScale(1, BigDecimal.RoundingMode.HALF_UP).toDouble,
          BigDecimal(row._4).setScale(1, BigDecimal.RoundingMode.HALF_UP).toDouble
     ))
      .orderBy($"working", $"sex", $"age")
  }
}
